...
## A-020 — MOCO-Integration: Polling statt Webhooks, Read-only-Key, Watermark-Semantik (2026-07-24)
Anbindung der MOCO-Agentursoftware (mocoapp.com): Rechnungen und Mahnungen werden automatisch als Brief versendet. Nicht-offensichtliche Festlegungen:
- **Credentials wie E-Post:** `moco_accounts` ist service-role-only (keine Client-RLS-Policies); API-Key AES-GCM-verschlüsselt (`EPOST_CREDENTIALS_KEY`). Ein **Read-only Account-Key** genügt — Versendio schreibt nichts nach MOCO (kein Status-Write-back, siehe I-032). Subdomain strikt als DNS-Label validiert, Mahnungs-`file_url` auf den Konto-Host gepinnt (SSRF).
- **Polling (Cron alle 10 min) statt Webhooks:** Serverless-Konvention des Projekts (keine Webhook-Empfänger nötig, MOCO-Rate-Limit 120 Req/2 min wird durch Batches ≤10 Dokumente/Konto/Tick weit unterschritten). Zeitbudget 45 s, Konten-Isolation nach Flows-Muster.
- **Exactly-once:** Claim per Insert in `moco_documents` (Unique `(user_id, doc_type, moco_id)`), Zeilen-ID = `p_client_token` von `confirm_send_job` (Flows-Enrollment-Präzedenz). Crash zwischen Claim/Brief/RPC wird beim nächsten Tick über den `pending`-Resume-Pfad idempotent fortgesetzt.
- **Watermark:** Nur Dokumente mit Datum ab Aktivierung des Auto-Versands (`activated_at`); Re-Aktivierung nach Komplett-Aus setzt die Marke neu — eine Pause versendet nie rückwirkend das Archiv. Mahnungen nur mit Status `created` (in MOCO noch nicht per E-Mail versendet).
- **Adresse:** MOCOs Freitext-`recipient_address` wird deterministisch geparst (PLZ-Zeile bottom-up, Länder-Präfixe/Zeilen, NL-Format) und gegen `contactSchema` validiert; nicht parsebare Adressen schlagen **laut** fehl (Aktivitätsliste + Digest) statt zu raten. Falsch adressierte Briefe sind teurer als übersprungene.
- **Guthaben:** Abbuchung über den regulären `confirm_send_job`-Pfad. Bei `insufficient_funds` wird das Dokument als fehlgeschlagen markiert und der Digest weist actionCritical darauf hin; kein automatischer Retry (Nutzer lädt auf und stößt manuell an). Bewusst einfach — Backoff-Retry siehe I-033.
- **Trennen löscht nur die Verbindung**, nicht das Dokumenten-Ledger — es bleibt Dedup-Anker, damit ein Reconnect nichts doppelt versendet.
