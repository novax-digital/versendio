...
type NavItem = {
  ...
  exact: boolean;
  /** Indented sub-item (e.g. Leadlisten under Kontakte). */
  child?: boolean;
};
...
const items: NavItem[] = [
  ...
  { href: "/app/leadlisten", label: de.nav.leadLists, icon: ListChecks, exact: false, child: true },
...
function LinkList({ isAdmin, onNavigate }: { isAdmin: boolean; onNavigate?: () => void }) {
  ...
  return (
    <>
      {allItems.map(({ href, label, icon: Icon, exact, child }) => {
        ...
        return (
          ...
          >
            <Icon className={child ? "size-3.5" : "size-4"} aria-hidden />
            {label}
          </Link>
        );
      })}
